...
<div class="row g-0 ">
  ...
  <div class="col-md-9 Hero mt-5 ">
    ...
    <form method="post" action="forecast.php" class="row" id="data-column" onsubmit="validateForm()">
      ...
      <div class="col-6 text-end">
        <button class="btn btn-warning" id="forecast">Ramal <i class="bi bi-chevron-right"></i></button>
      </div>
    </form>
  </div>
</div>
...
<script type="text/javascript">
  ...
  $tr.each(function() {
    ...
    checkBlock =
      checkBlock +
      '<label><input type="checkbox" name="check" checked onChange=""checkboxStatusChange() class="checkblock" thClass="' +
      id + '" value="' + colName + '"/> ' + colName + '</label>';
    count++;
  });
...
<script type="text/javascript">
  $(document).ready(function() {
    $("#data-column").submit(function(e) {
      // only choose checked 2 checkbox
      ...
      if (checked == 2) {
        ...
        var alpha = $("#alpha").val();
        // get index of checked checkbox
        var checkedIndex = $('input[type=checkbox]:checked').map(function() {
          return $(this).attr("thClass");
        }).get();
        if (alpha == "") {
          createCookie("column", checkedValues);
          createCookie("alpha", 0.1);
          createCookie("index", checkedIndex)
        } else {
          createCookie("column", checkedValues);
        }
      } else {
